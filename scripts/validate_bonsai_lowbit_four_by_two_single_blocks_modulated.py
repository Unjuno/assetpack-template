...
ROOT = Path('reports')
SOURCE = ROOT / 'bonsai-lowbit-four-by-two-single-blocks-modulated-onnx' / 'report.json'
OUT = ROOT / 'bonsai-lowbit-four-by-two-single-blocks-modulated-validation' / 'report.json'
...
CRITICAL_OUTPUTS = [
    ...
    'segment4_5_block0_output',
    'segment4_5_block1_output',
    'segment6_7_block0_output',
    'segment6_7_block1_output',
    'chained_final_block7_output',
]
DIAGNOSTIC_SUFFIXES = [
...
def main() -> None:
    ...
    require(source.get('graph_kind') == 'four_by_two_single_blocks_modulated_attention_to_out_residual_stack', f'unexpected graph kind: {source}')
    require(source.get('is_four_by_two_single_blocks_modulated_stack') is True, f'missing four-by-two flag: {source}')
    ...
    require(int(source.get('sequence_block_count', 0)) == 8, f'bad sequence block count: {source}')
    require(int(source.get('onnx_segment_count', 0)) == 4, f'bad onnx segment count: {source}')
    ...
    require(residual.get('method') == 'sequential_hidden_plus_gate_times_to_out_across_four_onnx_segments', f'bad residual method: {source}')
    require(residual.get('final_output') == 'block7_output', f'bad final output: {source}')
    ...
    require(len(layers) == 8, f'expected eight lowbit layer entries: {source}')
