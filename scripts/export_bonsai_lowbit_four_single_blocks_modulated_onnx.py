...
OUT_DIR = Path("reports/bonsai-lowbit-four-single-blocks-modulated-onnx")
SEQ_LEN = 4
...
class FourSingleBlocksModulated(torch.nn.Module):
    def __init__(self, cores: list[SingleBlockModulatedCore]):
        ...
        for core in cores:
            ...
            if int(core.num_heads) != self.num_heads or int(core.head_dim) != self.head_dim:
                raise ValueError("head schema mismatch across cores")

    def forward(self, hidden: torch.Tensor, temb: torch.Tensor) -> tuple[torch.Tensor, ...]:
        h = hidden
        block_outputs = []
        semantic_inputs = []
        gates = []
        ...
        for core in self.cores:
            ...
            out = core(h, temb)
            h = out[4]
            block_outputs.append(out[4])
            semantic_inputs.append(out[1])
            gates.append(out[5])
            weights.append(out[6])
        return tuple(block_outputs + semantic_inputs + gates + weights)
...
def main() -> int:
    ...
    module = FourSingleBlocksModulated([build_core(sd, index) for index in BLOCK_INDICES]).eval()

    generator = torch.Generator(device="cpu")
    generator.manual_seed(270_000)
    ...
    onnx_path = OUT_DIR / "lowbit_four_single_blocks_modulated.onnx"
    output_names = (
        [f"block{index}_output" for index in BLOCK_INDICES]
        + [f"block{index}_semantic_to_out_input" for index in BLOCK_INDICES]
        + [f"block{index}_gate" for index in BLOCK_INDICES]
        + [f"block{index}_weights" for index in BLOCK_INDICES]
    )
    dynamic_axes = {"hidden": {0: "batch", 1: "seq"}, "temb": {0: "batch"}}
    for name in output_names:
        if name.endswith("_weights"):
            dynamic_axes[name] = {0: "batch", 2: "query_seq", 3: "key_seq"}
    ...
    for index in BLOCK_INDICES:
        expected_shapes[f"block{index}_output"] = [bsz, seq, module.hidden_dim]
        expected_shapes[f"block{index}_semantic_to_out_input"] = [bsz, seq, module.semantic_input_width]
        expected_shapes[f"block{index}_gate"] = [bsz, module.hidden_dim]
        expected_shapes[f"block{index}_weights"] = [bsz, module.num_heads, seq, seq]
    ...
    report = {
        ...
        "graph_kind": "four_single_blocks_modulated_attention_to_out_residual_stack",
        "is_four_single_blocks_modulated_stack": True,
        ...
        "residual_schema": {"method": "sequential_hidden_plus_gate_times_to_out", "inputs": ["hidden", "block0_output", "block1_output", "block2_output"], "final_output": "block3_output", "block_output_width": module.hidden_dim},
