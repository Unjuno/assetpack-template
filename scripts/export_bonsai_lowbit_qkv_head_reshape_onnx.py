...
OUT_DIR = Path("reports/bonsai-lowbit-qkv-head-reshape-onnx")
HEAD_DIM_CANDIDATES = [128, 64]


class QkvHeadReshapeProjection(torch.nn.Module):
    ...
    def _to_heads(self, x: torch.Tensor) -> torch.Tensor:
        # [batch, hidden] -> [batch, heads, head_dim]
    ...
    def forward(self, hidden: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        q_heads = self._to_heads(q)
        k_heads = self._to_heads(k)
        v_heads = self._to_heads(v)
...
def main() -> int:
    ...
    module = QkvHeadReshapeProjection(proj).eval()

    hidden = torch.randn(2, module.hidden_dim, dtype=torch.float32) / 10.0
    with torch.inference_mode():
        y_pt = [y.detach().cpu().numpy() for y in module(hidden)]

    onnx_path = OUT_DIR / "lowbit_qkv_head_reshape.onnx"
    output_names = ["q_heads", "k_heads", "v_heads", "mlp"]
    torch.onnx.export(
        module,
        (hidden,),
        str(onnx_path),
    ...
    expected_shapes = {
        "q_heads": [int(hidden.shape[0]), module.num_heads, module.head_dim],
        "k_heads": [int(hidden.shape[0]), module.num_heads, module.head_dim],
        "v_heads": [int(hidden.shape[0]), module.num_heads, module.head_dim],
        "mlp": [int(hidden.shape[0]), module.mlp_size],
    }
    outputs = []
    allclose = True
    ...
    report = {
        ...
        "layer": LAYER,
        "graph_kind": "qkv_projection_split_and_head_reshape",
        "is_attention": False,
        "is_real_transformer_block": False,
        "hidden_dim": module.hidden_dim,
        "projection_output_dim": module.out_features,
        "head_schema": {
            "method": "reshape_qkv_hidden_dim_to_batch_heads_head_dim",
        ...
        "claim": "qkv_projection_head_reshape_onnxruntime_cpu_verified_not_attention_or_transformer_block",
    }
