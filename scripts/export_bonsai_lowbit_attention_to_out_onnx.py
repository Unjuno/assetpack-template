...
OUT_DIR = Path("reports/bonsai-lowbit-attention-to-out-onnx")
SEQ_LEN = 4


class AttentionToOut(torch.nn.Module):
    ...
    def forward(self, hidden: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        return context_flat, mlp_swiglu, semantic_to_out_input, to_out_output, weights, scores
...
def main() -> int:
    ...
    onnx_path = OUT_DIR / "lowbit_attention_to_out.onnx"
    output_names = ["context_flat", "mlp_swiglu", "semantic_to_out_input", "to_out_output", "weights", "scores"]
    torch.onnx.export(
        module,
        (hidden,),
        str(onnx_path),
    ...
    report = {
        ...
        "layer": QKV_MLP_PROJ,
        "to_out_layer": TO_OUT,
        "graph_kind": "qkv_projection_attention_to_out",
        "is_attention_math": True,
        "is_attention_to_out_path": True,
        ...
        "to_out_schema": {"layer": TO_OUT, "in_features": module.to_out_in_features, "out_features": module.to_out_out_features, "connected": True, "output_width": module.to_out_out_features},
    ...
    print(json.dumps({"graph_kind": report["graph_kind"], "to_out_schema": report["to_out_schema"], "semantic_input_schema": report["semantic_input_schema"], "max_abs_error": report["max_abs_error"], "all_outputs_allclose": report["all_outputs_allclose_rtol_1e_4_atol_1e_5"]}, indent=2))
