...
OUT_DIR = Path("reports/bonsai-lowbit-ten-by-two-single-blocks-modulated-onnx")
STRICT_RTOL = 1e-4
STRICT_ATOL = 1e-5
...
def output_category(name: str) -> str:
    return "critical" if name.endswith("_output") or name == "chained_final_block19_output" else "diagnostic"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bonsai-ten-by-two-") as tmp:
        tmp_dir = Path(tmp)
    ...
    report = {
        ...
        "residual_schema": {"method": "sequential_hidden_plus_gate_times_to_out_across_ten_onnx_segments", "segment_inputs": ["hidden"] + [f"segment{a}_{b}_block1_output" for a, b in SEGMENTS[:-1]], "final_output": "block19_output", "block_output_width": hidden_dim},
