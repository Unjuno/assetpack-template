...
ROOT = Path('reports')
SOURCE = ROOT / 'bonsai-lowbit-two-by-two-single-blocks-modulated-onnx' / 'report.json'
...
def main() -> None:
    ...
    require(source.get('graph_kind') == 'two_by_two_single_blocks_modulated_attention_to_out_residual_stack', f'unexpected graph kind: {source}')
    require(source.get('is_two_by_two_single_blocks_modulated_stack') is True, f'missing two-by-two flag: {source}')
    ...
    require(int(source.get('onnx_segment_count', 0)) == 2, f'bad onnx segment count: {source}')
    ...
    require(residual.get('method') == 'sequential_hidden_plus_gate_times_to_out_across_two_onnx_segments', f'bad residual method: {source}')
    require(residual.get('segment0_input') == 'hidden', f'bad segment0 input: {source}')
