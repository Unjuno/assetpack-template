...
OUT_DIR = Path("reports/bonsai-lowbit-attention-math-onnx")
HEAD_DIM_CANDIDATES = [128, 64]
SEQ_LEN = 4


class AttentionMathProjection(torch.nn.Module):
    ...
    def forward(self, hidden: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        return context, weights, scores, q_heads, k_heads, v_heads
...
def main() -> int:
    ...
    module = AttentionMathProjection(proj).eval()
    ...
    onnx_path = OUT_DIR / "lowbit_attention_math.onnx"
    output_names = ["context", "weights", "scores", "q_heads", "k_heads", "v_heads"]
    torch.onnx.export(
        module,
        (hidden,),
        str(onnx_path),
        ...
        dynamic_axes={
            ...
            "context": {0: "batch", 2: "seq"},
            "weights": {0: "batch", 2: "query_seq", 3: "key_seq"},
            "scores": {0: "batch", 2: "query_seq", 3: "key_seq"},
            "q_heads": {0: "batch", 2: "seq"},
            "k_heads": {0: "batch", 2: "seq"},
            "v_heads": {0: "batch", 2: "seq"},
        },
    )
    ...
    expected_shapes = {
        "context": [bsz, module.num_heads, seq, module.head_dim],
        "weights": [bsz, module.num_heads, seq, seq],
        "scores": [bsz, module.num_heads, seq, seq],
        "q_heads": [bsz, module.num_heads, seq, module.head_dim],
        "k_heads": [bsz, module.num_heads, seq, module.head_dim],
        "v_heads": [bsz, module.num_heads, seq, module.head_dim],
    }
    outputs = []
    allclose = True
    ...
    report = {
        ...
        "layer": LAYER,
        "graph_kind": "qkv_projection_scaled_dot_product_attention_math",
        ...
        "attention_math": {
            "score_formula": "matmul(q, transpose(k)) / sqrt(head_dim)",
            "softmax_dim": -1,
            "context_formula": "matmul(softmax(scores), v)",
            "mask": None,
        },
        "head_schema": {
        ...
        "claim": "qkv_projection_scaled_dot_product_attention_math_onnxruntime_cpu_verified_not_to_out_or_transformer_block",
    }
    ...
    print(json.dumps({
        ...
        "attention_math": report["attention_math"],
