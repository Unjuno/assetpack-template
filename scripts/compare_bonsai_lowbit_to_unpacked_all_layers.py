...
OUT_DIR = Path("reports/bonsai-lowbit-compare-all-layers")
...
def choose_prefixes(sd: dict, ref_keys: set[str]) -> list[str]:
    ...
    for key in sorted(sd.keys()):
        if not key.endswith(".W_q"):
            continue
        prefix = key[:-4]
        if not all(f"{prefix}.{name}" in sd for name in ["W_q", "scales", "zeros", "orig_shape", "metadata"]):
            continue
...
def main() -> int:
            ...
            try:
                ...
                if list(low_weight.shape) != list(ref_weight.shape):
                    if list(low_weight.t().shape) == list(ref_weight.shape):
                        ...
                        raise ValueError(f"shape mismatch low={list(low_weight.shape)} ref={list(ref_weight.shape)}")
    ...
    report = {
        ...
        "total_compared_layers": len(results),
        "exact_equal_count": sum(1 for r in results if r.get("exact_equal") is True),
        "failure_count": len(failures),
        "all_exact_equal": len(failures) == 0,
    ...
    print(json.dumps({"total_compared_layers": report["total_compared_layers"], "failure_count": report["failure_count"], "all_exact_equal": report["all_exact_equal"], "max_abs_error": report["max_abs_error"]}, indent=2))
    if failures:
        return 1
    return 0
