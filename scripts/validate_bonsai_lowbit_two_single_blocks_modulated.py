...
ROOT = Path('reports')
SOURCE = ROOT / 'bonsai-lowbit-two-single-blocks-modulated-onnx' / 'report.json'
OUT = ROOT / 'bonsai-lowbit-two-single-blocks-modulated-validation' / 'report.json'
...
def main() -> None:
    ...
    require(source.get('graph_kind') == 'two_single_blocks_modulated_attention_to_out_residual_stack', f'unexpected graph kind: {source}')
    require(source.get('is_two_single_blocks_modulated_stack') is True, f'missing stack flag: {source}')
    require(source.get('block_indices') == [0, 1], f'bad block indices: {source}')
    require(int(source.get('sequence_block_count', 0)) == 2, f'bad sequence block count: {source}')
    ...
    require(residual.get('block0_input') == 'hidden', f'bad block0 input: {source}')
    require(residual.get('block1_input') == 'block0_output', f'bad block1 input: {source}')
    ...
    require(len(layers) == 2, f'expected two lowbit layer entries: {source}')
    for expected, item in zip([0, 1], layers):
    ...
    for name in ['block0_output', 'block1_output', 'block0_semantic_to_out_input', 'block1_semantic_to_out_input', 'block0_gate', 'block1_gate', 'block0_weights', 'block1_weights']:
