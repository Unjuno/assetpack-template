...
OUT_DIR = Path("reports/bonsai-lowbit-four-by-two-single-blocks-modulated-onnx")
STRICT_RTOL = 1e-4
STRICT_ATOL = 1e-5
...
def output_category(name: str) -> str:
    return "critical" if name.endswith("_output") or name == "chained_final_block7_output" else "diagnostic"
...
def main() -> int:
    ...
    pt_segment_outputs: list[list[np.ndarray]] = []
    h_pt = hidden
    with torch.inference_mode():
        for module in modules:
            outs = [y.detach().cpu().numpy() for y in module(h_pt, temb)]
            pt_segment_outputs.append(outs)
            h_pt = torch.from_numpy(outs[1]).to(dtype=hidden.dtype)

    ort_segment_outputs: list[list[np.ndarray]] = []
    segment_sizes: list[int] = []
    output_names: list[list[str]] = []
    with tempfile.TemporaryDirectory(prefix="bonsai-four-by-two-") as tmp:
        tmp_dir = Path(tmp)
        segment_paths = [tmp_dir / f"segment_{a}_{b}.onnx" for a, b in SEGMENTS]
        ...
        for path in segment_paths:
            ...
            outs = session.run(None, {"hidden": h_ort, "temb": temb_np})
            ort_segment_outputs.append(outs)
            h_ort = outs[1].astype(np.float32)
        segment_sizes = [path_size(path) for path in segment_paths]

    outputs = []
    for segment, names, ort_outs, pt_outs in zip(SEGMENTS, output_names, ort_segment_outputs, pt_segment_outputs):
        outputs.extend(compare_outputs(f"segment{segment[0]}_{segment[1]}", names, ort_outs, pt_outs))
    final_diff = ort_segment_outputs[-1][1] - pt_segment_outputs[-1][1]
    outputs.append({
        "name": "chained_final_block7_output",
        "category": "critical",
        "output_shape": list(pt_segment_outputs[-1][1].shape),
        "mean_abs_error": float(np.abs(final_diff).mean()),
        "max_abs_error": float(np.abs(final_diff).max()),
        "allclose_rtol_1e_4_atol_1e_5": bool(np.allclose(ort_segment_outputs[-1][1], pt_segment_outputs[-1][1], rtol=STRICT_RTOL, atol=STRICT_ATOL)),
        "diagnostic_allclose_rtol_1e_4_atol_1e_4": bool(np.allclose(ort_segment_outputs[-1][1], pt_segment_outputs[-1][1], rtol=DIAGNOSTIC_RTOL, atol=DIAGNOSTIC_ATOL)),
    })
    ...
    report = {
        ...
        "graph_kind": "four_by_two_single_blocks_modulated_attention_to_out_residual_stack",
        "is_four_by_two_single_blocks_modulated_stack": True,
        "block_indices": list(range(8)),
        "segment_block_indices": [list(segment) for segment in SEGMENTS],
        "sequence_block_count": 8,
        "onnx_segment_count": 4,
        ...
        "residual_schema": {"method": "sequential_hidden_plus_gate_times_to_out_across_four_onnx_segments", "segment_inputs": ["hidden", "segment0_1_block1_output", "segment2_3_block1_output", "segment4_5_block1_output"], "final_output": "block7_output", "block_output_width": hidden_dim},
